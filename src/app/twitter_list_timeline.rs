...
struct Inner {
    list_id: u64,
    response: Option<twitter::ResponseFuture<Vec<twitter::Tweet>>>,
    wait: Delay,
...
impl TwitterListTimeline {
        ...
        C::Future: 'static,
    {
        ...
        let response = if let Some(ref mut resp) = inner.response {
            resp
        } else {
            if is_elapsed(&mut inner.wait, cx)? {
                inner.send_request(core);
            }
            return Poll::Pending;
        };

        let result = ready!(response.poll_unpin(cx));
...
impl Inner {
        ...
        C::Future: 'static,
    {
        ...
        self.wait.reset(Instant::now() + Duration::from_secs(1));
...
fn is_elapsed(delay: &mut Delay, cx: &mut Context<'_>) -> Fallible<bool> {
    Ok(delay.is_elapsed() || delay.compat().poll_unpin(cx)?.is_ready())
}
